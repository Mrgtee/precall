  ...
  return (
    <main className="shell page info-page">
      ...
      <section className="info-grid">
        ...
        <article className="panel info-card"><h2>Circle stack</h2><p>x402 rail: {data.circleStack.x402RailStatusLabel || "Worker-managed on Railway"}</p><p>Network: {data.circleStack.x402PaymentNetworkLabel || data.circleStack.gatewayChain}</p><p>Gateway balance: {data.circleStack.gatewayAvailableUsdc ? usdc(data.circleStack.gatewayAvailableUsdc) : "available from Railway health"}</p><p>Daily spend: {usdc(data.counts?.x402Spend || 0)} / {usdc(data.circleStack.dailyBudgetUsdc)}</p></article>
