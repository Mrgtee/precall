...
contract PrecallRegistry {
    enum Direction { Unknown, BuyYes, BuyNo }
    ...
    event CallPublished(uint256 indexed callId, uint256 indexed agentId, address indexed publisher, string marketId, uint8 direction, uint256 bondAmount, uint256 unlockPrice);
    ...
    event CallResolved(uint256 indexed callId, bool outcomeYes, int256 realizedPnlBps, uint16 brierScoreBps, bool bondReturned);
    ...
    function publishCall(uint256 agentId, string calldata marketId, uint8 direction, uint16 marketPriceBps, uint16 agentProbabilityBps, uint16 confidenceBps, uint64 expiry, bytes32 thesisHash, bytes32 evidenceHash, uint256 bondAmount, uint256 unlockPrice) external returns (uint256 callId) {
        Agent storage agent = agents[agentId];
        ...
        callId = nextCallId++;
        calls[callId] = MarketCall({ agentId: agentId, marketId: marketId, direction: Direction(direction), marketPriceBps: marketPriceBps, agentProbabilityBps: agentProbabilityBps, confidenceBps: confidenceBps, expiry: expiry, thesisHash: thesisHash, evidenceHash: evidenceHash, bondAmount: bondAmount, unlockPrice: unlockPrice, publisher: msg.sender, resolved: false, outcomeYes: false, realizedPnlBps: 0, brierScoreBps: 0 });
    ...
    function resolveCall(uint256 callId, bool outcomeYes, int256 realizedPnlBps, uint16 brierScoreBps) external onlyResolver {
        MarketCall storage marketCall = calls[callId];
        ...
        bool directionCorrect = (marketCall.direction == Direction.BuyYes && outcomeYes) || (marketCall.direction == Direction.BuyNo && !outcomeYes);
